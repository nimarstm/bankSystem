  ...
  // source account info (use from localStorage if available, else placeholder)
  const accountNo = localStorage.getItem('account_number') || 'My Default Account';
  document.getElementById('source-display').textContent = accountNo;
  ...
  function formatIban(input) {
    let v = input.value.replace(/\s/g,'').toUpperCase().slice(0,34);
    input.value = v.replace(/(.{4})/g,'$1 ').trim();
  }
  ...
  function clearErrors() {
    ...
    ['card-dest','iban-dest','amount'].forEach(id => {
      const el = document.getElementById(id);
  ...
  function validate() {
    ...
    if (currentMethod === 'card') {
      ...
      if (!/^\d{16}$/.test(raw)) {
        fe('err-card-dest','Please enter a valid 16-digit card number.');
        document.getElementById('card-dest').classList.add('error');
        ok = false;
      }
    } else {
      const raw = document.getElementById('iban-dest').value.replace(/\s/g,'');
      if (raw.length < 15 || raw.length > 34) {
        fe('err-iban-dest','Please enter a valid IBAN (15–34 characters).');
        document.getElementById('iban-dest').classList.add('error');
        ok = false;
      }
    }
    ...
    if (!amount || amount <= 0) {
      fe('err-amount','Please enter a valid amount greater than €0.');
  ...
  function openConfirm() {
    ...
    const dest   = currentMethod === 'card'
      ? document.getElementById('card-dest').value.trim()
      : document.getElementById('iban-dest').value.trim();
  ...
  async function submitTransfer() {
    ...
    if (currentMethod === 'card') {
      ...
      payload  = { source: accountNo, destination: dest, amount, description: desc };
    } else {
    ...
    try {
      ...
      if (res.ok) {
        ...
        closeConfirm();
        showToast(data.detail || data.message || 'Transfer failed. Please try again.', 'error');
      }
    } catch {
      closeConfirm();
      showToast('Network error. Please try again.', 'error');
    } finally {
      btn.disabled=false; spin.style.display='none'; icon.style.display=''; txt.textContent='Send Money';
    }
  }
  ...
  function showSuccess(payload) {
    ...
    document.getElementById('success-sub').textContent   =
      `€${payload.amount.toFixed(2)} has been sent successfully.`;
    document.getElementById('success-detail').innerHTML  = `
      <div class="success-detail-row"><span>Method</span><span>${currentMethod==='card'?'Card Transfer':'IBAN Transfer'}</span></div>
      <div class="success-detail-row"><span>To</span><span>${payload.destination}</span></div>
      <div class="success-detail-row"><span>Amount</span><span>€${payload.amount.toFixed(2)}</span></div>
      ${payload.description ? `<div class="success-detail-row"><span>Description</span><span>${payload.description}</span></div>` : ''}
      <div class="success-detail-row"><span>Date</span><span>${new Date().toLocaleString()}</span></div>
    `;
