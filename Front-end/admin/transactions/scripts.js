  ...
  function renderDrawer(t){
    ...
    document.getElementById('drawer-footer').innerHTML=canReverse
      ?`<button class="btn danger" onclick="openReverseModal()"><i class="ti ti-rotate-clockwise-2"></i> Reverse Transaction</button>`
      :`<span style="font-size:12.5px;color:var(--text-3);">This transaction cannot be reversed.</span>`;
  }
  ...
  async function confirmReverse(){
    ...
    try{
      ...
      if(res.ok){ toast('Transaction reversed'); closeReverseModal(); closeDrawer(); loadTx(currentPage); }
